...
export const fetchtiffinSearchCards = createAsyncThunk(
    ...
    async (data, thunkAPI) => {
        ...
        try {
            const response = await api.get(`${BASE_URL}/search-vendors?search_term=${people}&limit=7&current_page=1&save_filter=1&vendor_type=Tiffin&app_type=web&latitude=${locationValuesGlobal?.latitude || ""}&longitude=${locationValuesGlobal?.longitude || ""}&city=${locationValuesGlobal?.city?.long_name || ""}&pincode=${locationValuesGlobal?.pincode || ""}&place_id=${locationValuesGlobal?.place_id || ''}&price_ranges=${JSON.stringify(updatedPriceTypes_formatted)}&food_types_filter=${JSON.stringify(foodtype_filter_formatted)}&order_by_filter=${cateringSortBy_filter}&subscription_types_filter=${JSON.stringify(subscriptionTypes_formatted)}&service_types_filter=${JSON.stringify(servicetype_filter_formatted)}&start_date=${moment(startDate).format('YYYY-MM-DD')}&end_date=${moment(endDate).format('YYYY-MM-DD')}`, {
                headers: {
                    authorization: `Bearer ${thunkAPI.getState()?.user?.accessToken}`,
                },
            });
...
export const tiffinFilterSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchtiffinSearchCards.fulfilled, (state, { payload }) => {
                state.isLoading = false;
                state.getTiffinSearchCards = payload;
            })
